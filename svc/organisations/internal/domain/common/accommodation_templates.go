...
type AccommodationConfig struct {
	Name string `bson:"name" validate:"required,min=3"`
	MinOccupancy int    `bson:"min_occupancy" validate:"required,min=1"`
	MaxOccupancy int   `bson:"max_occupancy" validate:"required,gtefield=MinOccupancy"`
...
func (ac *AccommodationConfig) IsValid() bool {
	if ac.MaxOccupancy < ac.MinOccupancy {
		return false
	}
